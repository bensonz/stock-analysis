...
- Decision: `input/intersect.json` is the working pool: `crawl.json ∩` the local RPS-filtered universe, with local RPS values merged onto the remote strategy members that pass the filter.
- Decision: the local RPS filter for `intersect.json` is strict: `rps60 > 85`, `rps120 > 85`, and `rps250 > 85`.
...
- Artifact contract: `input/crawl.json` records the raw CheeseFortune list for the day; `input/intersect.json` records only the remote names that also satisfy the local RPS filter; `input/strategy_pool_debug.json` explains the intersection counts and drops, including threshold rejects.
